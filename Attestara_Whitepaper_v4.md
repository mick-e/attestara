...
A **Principal** (organisation or authorised human) issues an **Authority Credential** to an Agent. This credential is a W3C Verifiable Credential (VC) conforming to the `AgentAuthorityCredential` type defined in the Attestara extension context (`https://attestara.io/contexts/v1`), which cryptographically encodes:
...
```jsonc
{
  "@context": [
    ...
    "https://attestara.io/contexts/v1"           // Attestara extension context (published)
  ],
  ...
  "proof": {
    ...
    "cryptosuite": "ecdsa-jcs-2019",                // W3C VC Data Integrity 1.0 registered cryptosuite (secp256k1 curve)
...
> **VC 2.0 Conformance.** The `@context` URL is `https://www.w3.org/ns/credentials/v2` (VC 2.0). The `validFrom`/`validUntil` properties are top-level credential properties per VC 2.0 §4.8. The proof uses `DataIntegrityProof` with the `ecdsa-jcs-2019` cryptosuite (a registered W3C VC Data Integrity cryptosuite supporting secp256k1) per VC Data Integrity 1.0. Revocation uses `BitstringStatusListEntry` per W3C Bitstring Status List v1.0. The Attestara extension context **MUST** be published at `https://attestara.io/contexts/v1` prior to DIF and W3C CCG Work Item submissions; until publication, this URL is reserved and does not yet resolve.
...
**Credential signatures (near-term, Phase 3):** The `DataIntegrityProof` cryptosuite will be migrated from `ecdsa-jcs-2019` (secp256k1) to `eddsa-jcs-2022` (Ed25519) as an intermediate step, followed by an ML-DSA cryptosuite when one is defined by the W3C VC Data Integrity specification family or the Attestara community. No W3C work item for an ML-DSA cryptosuite currently exists; NIST FIPS 204 (ML-DSA) is finalised but the W3C Data Integrity binding has not yet been initiated. Ed25519 offers improved classical security properties and is widely supported in the VC ecosystem.
...
> **Standards note for DIF and W3C CCG reviewers:** This PQC migration path is flagged as a required section in all standards body submissions. The VC Data Integrity specification family is expected to publish ML-DSA cryptosuite specifications by 2027. Attestara commits to tracking these specifications and updating the `AgentAuthorityCredential` proof requirements accordingly.
...
**5.4.1 Credential Issuance (DIF Issue Credential v3)**

Authority Credentials are issued using the DIF Issue Credential v3 protocol over DIDComm v2. The issuance flow is:
...
The `issue-credential` message body **MUST** contain the `AuthorityCredential` VC as a DIDComm v2 attachment with `media_type: "application/vc"`. The credential body **MUST** be encrypted using the Agent's DID key (ECDH-1PU + XC20P) so that only the intended Agent can decrypt the mandate parameters.
...
```json
{
  "type": "https://attestara.io/protocols/session/1.0/invite",
  "id": "<message-uuid>",
  ...
  "body": {
    ...
    "expiresAt": "2026-03-22T01:00:00Z"
  }
}
```
...
```json
{
  "type": "https://attestara.io/protocols/negotiation/1.0/turn",
  "id": "<message-uuid>",
...
The DIF Credential Manifest [37] for `AgentAuthorityCredential` issuance defines what a Principal **MUST** provide to receive a credential. This enables composability with DIF-compatible issuance infrastructure:
...
```json
{
  ...
  "output_descriptors": [
    {
      "id": "agent-authority-credential",
      "schema": "https://attestara.io/schemas/AgentAuthorityCredential/v1"
    }
  ],
...
- **DID key security:** secp256k1 security. Mitigation: HSM; key rotation (Section 5.2). PQC migration path: Section 4.7.
...
| VC Data Integrity | v1.0 (Rec. May 2025) | `DataIntegrityProof` with `ecdsa-jcs-2019` (secp256k1 curve) |
...
| 16 | **W3C CCG** | Submit `did:attestara` DID method specification for CCG registry listing (under consideration; would provide agent-specific DID resolution with embedded mandate metadata pointers, complementing did:ethr for deployments requiring richer DID document semantics) |
...
const client = new AttestaraClient({
  ...
  relayUrl: 'wss://relay.attestara.io',       // DIDComm v2 relay
});
...
- **Extension context published at `https://attestara.io/contexts/v1`**
...
- Ed25519 (`eddsa-jcs-2022`) proof migration (PQC interim step)
...
**Cryptographic agility.** Attestara's current implementation uses secp256k1 for DID keys and credential signatures, and Groth16 (BN254 pairing) for ZK proofs. Both are classical cryptographic systems. The PQC migration path in Section 4.7 provides a roadmap for transitioning to quantum-resistant primitives. Implementers deploying Attestara for long-term commitment records (multi-year archives) **SHOULD** monitor the PQC migration timeline and plan re-signing accordingly.
