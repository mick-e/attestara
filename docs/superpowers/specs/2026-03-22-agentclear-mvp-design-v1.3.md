...
MVP commitment records are technical proofs of agreement between AI agents. They are **NOT** automatically legally binding contracts. The legal enforceability analysis in `05-legal-enforceability.md` identifies open questions requiring external counsel opinion before any commitment is represented as legally binding to pilot customers or investors. This caveat must be reflected in portal UI copy, investor demo scripts, and pilot customer onboarding materials.

### Pricing Tiers (Placeholder — to be finalised before investor demos)

| Tier | Target | Session Volume | Proof Volume | Price (indicative) |
|------|--------|---------------|-------------|-------------------|
| Starter | Early adopters / developers | 500/month | 2,000/month | ~$499/month |
| Growth | Mid-market enterprises | 5,000/month | 20,000/month | ~$1,999/month |
| Enterprise | Large orgs / custom SLAs | Unlimited | Unlimited | Custom / annual contract |

Finalize numbers with commercial counsel and investor feedback before publishing on `/pricing`.
...
### Cross-Org Negotiation Model

The primary B2B use case is Agent A (Org 1) negotiating with Agent B (Org 2). This is architecturally distinct from intra-org sessions and must be resolved before Track 2 begins implementation.

**Session authorization:** A cross-org session is initiated by the initiating org and requires explicit acceptance by the counterparty org before turns begin. The relay issues a session invite token (short-lived JWT scoped to `session:accept`) sent out-of-band (or via a `/sessions/:sessionId/invite` endpoint) to the counterparty org admin.

**Multi-tenancy boundary:** Cross-org sessions are visible to both orgs but each org sees only its own agent's mandate data. The `sessions` table gains a `session_type ENUM(intra_org, cross_org)` column. ORM-layer queries for session listing filter by `initiator_org_id = $orgId OR counterparty_org_id = $orgId`. Turn data is scoped so each org can only read turns produced by its own agents plus the public signals of counterparty turns.

**Trust model:** Both agents must be registered in `AgentRegistry` before a cross-org session can be anchored on-chain. The relay verifies both DIDs resolve to registered agents at session creation time.

**Schema addition:**
```sql
sessions: add columns
  initiator_org_id  UUID FK→organisations  -- org owning the initiating agent
  counterparty_org_id UUID FK→organisations -- org owning the counterparty agent
  session_type      ENUM(intra_org, cross_org) DEFAULT intra_org
  invite_token_hash TEXT  -- hashed invite token; cleared on acceptance
```
...
- **Note:** Full EdDSA verification inside Circom is constraint-heavy. If benchmarks exceed 5s, the fallback approach is: sign challenge externally and verify signature on-chain instead of inside ZK circuit, trading ZK privacy for performance.

  **Fallback spec (implement if Week 5 benchmarks exceed 5s):**
  - Agent signs `keccak256(challenge || sessionId || agentId)` using its Baby JubJub private key off-circuit
  - Relay submits `(signature, publicKey, challenge)` to a lightweight `SignatureVerifier` contract (ecrecover-equivalent for BabyJubJub)
  - `did_commitment` is replaced by `publicKeyHash` in public inputs — the DID binding is proven by the registry lookup, not ZK
  - Privacy trade-off: the agent's public key is visible to the verifier. Acceptable for MVP; ZK version re-enabled once constraint optimisations are complete
  - Contractor must stub the `IdentityBindingVerifier` interface identically in both paths so `CommitmentContract` requires no changes
...
Each circuit is versioned independently. The `CommitmentContract` stores a `circuitVersion` alongside each proof. Verifier contracts are immutable once deployed; upgrades deploy a new verifier at a new address and update a `verifierRegistry` mapping in `CommitmentContract`.

```solidity
// In CommitmentContract
mapping(bytes32 => address) public verifiers; // circuitId+version → verifier address
// e.g. keccak256("MandateBound:v1") → 0xabc...
```

Old commitments remain verifiable via their original verifier address (stored in `CommitmentRecord`). New commitments use the latest registered verifier. A `CircuitVersionDeprecated` event is emitted when an old version is flagged as insecure; the portal UI shows a warning badge on affected commitments.

The 48-hour timelock governs verifier registry updates, giving stakeholders time to review before new circuit versions go live on mainnet.

### Under-Constrained Circuit Risk

Circom circuits can accept invalid proofs if constraints are under-specified — a subtle and common bug class. Mitigations required from the contractor:

- Run `circom --inspect` on all circuits and document constraint count vs. expected (from spec) before delivery
- Each circuit's test suite must include at least one **soundness test**: a witness that violates the stated constraint (e.g., `proposed > max_value` for MandateBound) and must fail proof generation
- The contractor code review (Week 8, optional Solidity reviewer) should prioritise circuit soundness over gas optimisation
- Formal verification (Certora) is deferred to Phase 2 — this is the accepted residual risk for MVP
...
createCommitment(sessionId, agreementHash, parties[],
    credentialHashes[], proofs[], publicSignals[], proofTypes[], signatures[])
...
Verifier routing: proofTypes[] is a parallel array to proofs[]. Each entry is a
bytes32 circuit identifier (e.g. keccak256("MandateBound:v1")). createCommitment()
looks up verifiers[proofTypes[i]] to resolve the correct Groth16Verifier address
for each proof, then calls verifyProof(proofs[i], publicSignals[i]). Reverts if
any verifier address is zero (unregistered circuit version).
...
- `ScriptedStrategy` — pre-scripted negotiation responses (use in tests and demos)
...
**REST endpoints (all prefixed `/v1/`):**

| Group | Routes |
|-------|--------|
| Auth | `POST /v1/auth/register, /v1/auth/login, /v1/auth/wallet, /v1/auth/refresh` |
| Orgs | `POST /v1/orgs`, `GET/PATCH /v1/orgs/:orgId`, `GET /v1/orgs/:orgId/members`, `POST /v1/orgs/:orgId/invite` |
| Agents | `POST/GET /v1/orgs/:orgId/agents`, `GET/PATCH/DELETE /v1/orgs/:orgId/agents/:agentId` |
| Credentials | `POST/GET /v1/orgs/:orgId/credentials`, `GET/DELETE /v1/orgs/:orgId/credentials/:id` |
| Sessions | `POST/GET /v1/sessions`, `GET /v1/sessions/:sessionId`, `GET /v1/sessions/:sessionId/turns`, `POST /v1/sessions/:sessionId/invite` (send cross-org invite), `POST /v1/sessions/:sessionId/accept` (accept invite, validates token) |
| Commitments | `GET /v1/commitments`, `GET /v1/commitments/:id`, `POST /v1/commitments/:id/verify` |
| Analytics | `GET /v1/analytics/overview, /sessions, /proofs, /costs` |
...
credentials (id UUID PK, org_id FK→organisations, agent_id FK→agents, credential_hash UNIQUE,
             schema_hash, ipfs_cid, credential_data_cached JSONB,  -- IPFS fallback
             mandate_params_encrypted BYTEA, mandate_key_version INT,  -- AES-256-GCM; key derived per-org from org master key
             expiry, revoked DEFAULT false,
             registered_tx_hash, created_at)

-- Negotiation
sessions (id UUID PK,
          initiator_agent_id FK→agents, initiator_org_id FK→organisations,
          counterparty_agent_id FK→agents, counterparty_org_id FK→organisations,
          session_type ENUM(intra_org, cross_org) DEFAULT intra_org,
          invite_token_hash TEXT,  -- cleared on acceptance; cross_org only
          status ENUM(pending_acceptance, active, completed, rejected, expired, committed),
         ...
         INDEX ON (initiator_org_id), INDEX ON (counterparty_org_id)  -- cross-org queries
...
commitments (id UUID PK, session_id FK→sessions UNIQUE, agreement_hash, parties UUID[],
             credential_hashes TEXT[], proofs JSONB, circuit_versions TEXT[],  -- parallel to proofs; for verifier lookup
             tx_hash, block_number, verified, created_at)
            INDEX ON (tx_hash)
...
-- Webhooks
webhooks (id UUID PK, org_id FK→organisations, url TEXT, secret_hash TEXT,
          events TEXT[],  -- e.g. ['session.completed','commitment.created']
          active BOOLEAN DEFAULT true, created_at)
webhook_deliveries (id UUID PK, webhook_id FK→webhooks, event TEXT, payload JSONB,
                    status ENUM(pending, delivered, failed), attempts INT DEFAULT 0,
                    last_attempted_at, delivered_at, created_at)
```

Redis handles: active WebSocket connections, session presence, pub/sub for turn delivery, proof verification cache.

**Chain indexer reliability:** On Render cold start or deploy, the indexer reads `last_processed_block` from Redis and backfills events from that block. To prevent runaway backfill during extended outages (e.g., 2+ hour relay downtime during investor demo), backfill is capped at 10,000 blocks per restart. If the gap exceeds this, an `INDEXER_LARGE_GAP` alert fires via the uptime monitor and an admin must trigger manual backfill via `POST /v1/admin/indexer/backfill`. This ensures the demo environment never stalls on restart.

**IPFS fallback:** The `credentials` table includes a `credential_data_cached JSONB` column containing the full VC content. On credential fetch, the relay first attempts IPFS retrieval (500ms timeout); on timeout or Pinata outage, it falls back to the cached column. This prevents Pinata availability from blocking live session demos or `isValid()` checks. The cache is written at credential registration time and is treated as read-only thereafter.
...
**Demo reliability — fallback mode:** The live path runs real ZK proofs against Arbitrum Sepolia. A `?mode=prerecorded` query param (and an automatic fallback if the RPC call fails within 3 seconds) switches to a pre-recorded session replay using `ScriptedStrategy` output. The portal UI is identical in both modes — the only difference is data source. This ensures the investor demo never fails due to testnet congestion or RPC downtime. The demo script should always use the live path first; presenters are briefed on the fallback toggle.
...
**Intra-org sessions:** Every database query includes `org_id` filter enforced at ORM layer. API keys scoped to org. JWT contains `orgId` claim. WebSocket connections scoped to org's sessions.

**Cross-org sessions:** ORM-layer queries for session listing use `initiator_org_id = $orgId OR counterparty_org_id = $orgId` (not a single `org_id` filter). Turn data access is further scoped: each org may read all `public_signals` from any turn (they are public by ZK design), but `terms JSONB` on counterparty turns is redacted at the API layer before delivery. The relay enforces this at response serialisation time, not at DB query time.

### Key Management

- Agent private keys: AES-256-GCM encrypted at rest, decrypted in memory for signing
- `mandate_params_encrypted`: AES-256-GCM encrypted using a per-org master key derived via HKDF from a root secret stored in environment variables (or AWS Secrets Manager for production pilots). A `mandate_key_version` column allows key rotation without re-encrypting all rows simultaneously. The plaintext mandate is never logged.
...
Relay → Prover communication uses a shared internal bearer token (`PROVER_INTERNAL_SECRET` env var). This is supplemented by network-level isolation: on Render, the prover service is configured as a **private service** (no public ingress), reachable only from the relay via Render's internal network. If Render private networking is unavailable in the chosen plan tier, the prover binds to `127.0.0.1` in local/Docker environments and uses firewall rules (UFW/iptables) to block external access in hosted environments. The `/prove/*` endpoints must never be publicly reachable.
...
| Week 4-5 | Trusted setup ceremony, benchmarks, generate Solidity verifiers. **Note for investor comms:** The MVP ceremony uses 2-5 participants (PoC-level). This is acknowledged and acceptable for MVP — the security assumption is that at least one participant is honest, which holds with known contributors. A production-grade ceremony (20+ participants, public MPC) is scheduled for Phase 2 and should be framed proactively in investor Q&A as a planned milestone, not a gap. |
...
- Relay stores last 100 events per session in Redis for replay on reconnect. Sessions are capped at 50 turns maximum (configurable in `session_config`), so 100 events comfortably covers the full session history plus system events. If the turn cap is raised in Phase 2, the replay buffer must be increased proportionally.
...
See Section 1 for the full legal notice. In summary: MVP commitment records are technical proofs, not automatically legally binding contracts. External counsel review is required before any commitment is represented as legally binding to customers or investors.
